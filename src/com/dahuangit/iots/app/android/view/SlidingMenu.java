 ...
 */
package com.dahuangit.iots.app.android.view;
			...
			if (mScroller.computeScrollOffset()) {
				...
				if (oldX != x || oldY != y) {
					if (mSlidingView != null) {
						...
						// 左边菜单和右边内容之间的边框宽度
						if (x < 0) {
							bgShade.scrollTo(x + 1, y);// 背景阴影右偏
						} else {
							bgShade.scrollTo(x - 1, y);// 背景阴影左偏
						}
					}
				}
				invalidate();
		...
		case MotionEvent.ACTION_MOVE:
			if (mIsBeingDragged) {
				...
				if (mSlidingView != null) {
					...
					// 滑动的时候左边菜单与右边边框大小
					if (scrollX < 0) {
						bgShade.scrollTo((int) scrollX + 1, mSlidingView.getScrollY());
					} else {
						bgShade.scrollTo((int) scrollX - 1, mSlidingView.getScrollY());
					}
				}
